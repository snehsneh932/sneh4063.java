...
class Matrices
{
	...
	void subtractMatrix(Matrices m)
	{
		int sub[][]=new int[r][c];
		for(int i=0;i<r;i++)
		{
			for(int j=0;j<c;j++)
			{
				sub[i][j]=this.mat[i][j]-m.mat[i][j];
			}
		}
...
class Main
{
	public static void main(String st[]) throws NumberFormatException,java.io.IOException
	{
		...
		{
			System.out.println();
			System.out.println("Subtracting matrix 1 and matrix 2 : ");
...
Enter value at index (1,1) : 9
Enter value at index (1,2) : 8
Enter value at index (2,1) : 7
Enter value at index (2,2) : 6
...
Enter value at index (2,2) : 2

Data in matrix :
5       4
3       2

Subtracting matrix 1 and matrix 2 :
Data after adding :
4       4
4       4
